...
test('control kill acknowledgement is optimistic (<200ms)', async (t) => {
  if (!(await canListenLoopback())) {
    t.skip('loopback listen is blocked in current sandbox');
    return;
  }

  const runtimeDir = await mkdtemp(path.join(os.tmpdir(), 'c2p-kill-latency-it-'));
  ...
  try {
    ...
    assert.ok(elapsedMs < 200, `kill acknowledgement should be <200ms, got ${elapsedMs.toFixed(2)}ms`);

    const sessionsPayload = await waitControlJson(
      (payload) => payload.type === 'sessions' && Array.isArray(payload.list) && !payload.list.some((item) => item.id === sessionId),
      1_000
    );
    assert.equal(Array.isArray(sessionsPayload.list), true);
