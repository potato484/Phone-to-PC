...
- [x] Phase 3: 桌面流（3.1 已完成并验收通过）
- [x] Phase 4: 文件管理 + 系统监控（4.1/4.2 已完成并验收通过）
- [ ] Phase 5: 长期方向
...
Phase 3 落地状态（2026-02-15）

- [x] `src/ws/desktop.ts`：新增 `/ws/desktop`，实现 WebSocket↔TCP VNC 透传代理
- [x] `src/vnc-manager.ts`：新增 VNC 管理器，支持端口探活 + x11vnc/wayvnc 自动拉起尝试 + 状态输出
- [x] `src/server.ts`：注册 desktop channel，静态暴露 `novnc` 资源目录（`/vendor/novnc/*`）
- [x] `public/lib/desktop.js`：接入 noVNC（RFB），落地模式切换、触控板/直点、Ctrl/Alt/Win、Ctrl+Alt+Del、帧率/压缩参数
- [x] `public/index.html` + `public/style.css`：新增桌面模式 UI 与交互控件

Phase 3 验收记录（2026-02-15）

- `npm run build` 通过
- noVNC 静态资源验收通过：`/vendor/novnc/core/rfb.js` 返回 `HTTP 200`
- 桌面通道握手验收通过：`/ws/desktop` 可建立并返回可预期关闭（无本机 VNC 时报 `1013`）
...
Phase 4 落地状态（2026-02-15）

- [x] 文件管理 API：`/api/fs/{list,read,write,mkdir,rename,remove,download,upload}` 全部落地
- [x] 路径安全：服务端统一 `path.resolve + startsWith(relative)` 防越界
- [x] 上传限制：`/api/fs/upload` 限制 100MB（超限返回 413）
- [x] 下载链路：`/api/fs/download` 流式输出
- [x] 系统监控 API：`/api/system/stats` 返回 CPU/内存/磁盘/网络/uptime + vnc 状态
- [x] 前端文件管理：目录列表、拖拽/按钮上传、文本编辑保存、长按操作菜单（打开/下载/重命名/删除）
- [x] 前端系统监控：dock 面板展示条形图与网络速率，5 秒轮询

Phase 4 验收记录（2026-02-15）

- `npm run build` 通过
- API smoke 通过：`/api/runtime`、`/api/fs/list`、`/api/system/stats`
- 文件链路 smoke 通过：`mkdir -> write -> read -> rename -> upload -> download -> remove`
- 前端语法验收通过：`node --check public/app.js public/lib/{state,control,term,ui,gestures,desktop,files,monitor}.js public/sw.js`
